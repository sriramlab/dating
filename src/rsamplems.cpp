...
rsamplems::rsamplems (string configfile) { 
	...
	while ( std::getline (inp, line)){
        ...
        oafs.push_back (atoi(toks[1].c_str ()));
    }
    inp.close();
...
/*
void rsamplems::readmshot (int reps){  
...
void rsamplems::readms (string filename){  
    ...
    cout << "Matched = " << countmatches << endl;
	snpfs.close ();
	if(inp.rdstate() == ios::eofbit)
		cout << "End of file!\n";
	if(inp.rdstate() == ios::badbit)
...
double rsamplems::nucdiv( int nsam, int segsites, char **list)
{
	int s;
...
/*   thetah - pi   */
double rsamplems::hfay( int nsam, int segsites, char **list)
{
	int s;
	double pi, p1, nd, nnm1  ;
...
/* Fay's theta_H  */
double rsamplems::thetah( int nsam, int segsites, char **list)
{
        int s;
        double pi, p1, nd, nnm1  ;
...
int rsamplems::frequency( char allele,int site,int nsam,  char **list)
{
        int i, count=0;
...
int rsamplems::segsub( int nsub, int segsites, char **list )
{
	int i, count = 0 , c1 ;
...
int main (int argc, char* argv[]) {
	if (argc > 1) { 
		rsamplems c = rsamplems(string(argv[1]));
/*		if (c.mshot)  
    		c.readmshot (c.reps);
		else 
    		c.readms (c.msfile);
