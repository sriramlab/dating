...
double loglygf (double y, void *param) { 
    getposterior *gp = (getposterior *)param;
    ...
    for (int i = 0 ; i  < an; i++) {
        double lambda = av[i] * log(y/(av[i]*gp->gen)+1);
        tmp[i] = gp->logd(gp->intercept,lambda, gp->sigma2);
    }
    double val  = vectorfn::lsumexp(tmp);
    val = val - log(y);
...
double loglgen (double g, void *param) { 
    getposterior *gp = (getposterior *)param;
    ...
    for (int i = 0 ; i  < an; i++) {
        double lambda = av[i] * log(gp->ygf/(av[i]*g)+1);
        tmp[i] = gp->logd(gp->intercept,lambda, gp->sigma2);
    }
    double val  = vectorfn::lsumexp(tmp);
    return (val);
...
//	double xinit[7] = {1,10,100,1000,1e4,1e5,1e6};
	...
	double xprev = ygf;
	double er = arms(xinit, ninit, &xl, &xr, loglygf, this,
			&convex,npoint, 1, &xprev, xsamp, nsamp, qcent, xcent, ncent, &neval);

	if (er > 0 ) { 
		cerr << "Error in sample_ygf: "  << er  << endl;
		exit (1);
	}
...
void getposterior::sample_gen () {
	double xl = 25;
	double xr = 33;
	int ninit  = 30;
//	double xinit[7] = {1,10,100,1000,1e4,1e5,1e6};
	double xinit[30] =	{ 25.5, 25.75, 26, 26.25, 26.5, 26.75, 27, 27.25, 27.5, 27.75, 28, 28.25, 28.5, 28.75, 29, 29.25, 29.5, 29.75, 30, 30.25, 30.5, 30.75, 31, 31.25, 31.5, 31.75, 32, 32.25, 32.5, 32.75};
	...
	double xprev = gen;
	double er = arms(xinit, ninit, &xl, &xr, loglgen, this,
			&convex,npoint, 1, &xprev, xsamp, nsamp, qcent, xcent, ncent, &neval);

	if (er > 0 ) { 
		cerr << "Error in sample_gen: "  << er  << endl;
		exit (1);
	}
...
double getposterior::mcmc_init () {
    ...
    io::println ("ygfinit = " + tostring(ygf),0);
    io::println ("geninit = " + tostring(gen),0);
...
//        sample_tgf ();
        ...
        if ( i%printint == 0 )  {
			if ( i < burnin)
                cout << "burnin:" << i << "\t" << intercept << "\t" << lambda <<  "\t" << sigma2 << "\t" << tgf << "\t" << alpha << "\t" << "\t" << gen << "\t" << ygf << endl;
            else {
                cout << "iter:" << i << "\t" << intercept << "\t" << lambda << "\t" << "\t" << sigma2 <<  "\t" << tgf << "\t" << alpha << "\t" <<gen << "\t" << ygf <<  endl;
                collect[0][j] = lambda;
                collect[1][j] = tgf;
                collect[2][j] = ygf;
                j++;
