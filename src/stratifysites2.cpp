...
stratifysites::stratifysites (int argc, char *argv[]) {
    ...
    if (givenindivfilterfile && givenindfile)
        geno = new genotype ( snpfile, indfile, genofile, "eigenstrat", false, true, indivfilterfile);
    else
        geno = new genotype ( snpfile, genofile, false, true);

    nind = geno->nind;
    nsnps = geno->nsnps;
    nchr = geno->nchr;

    predanc = new packedgtype (nind, nsnps, geno->cumnumsnps, false, false, "prob");
    predanc->read_eigenstrat (predfile, NULL);
    if (io::debug>=2) { 
    ...
    if (giventrue) { 
        ...
        trueanc->read_eigenstrat (truefile, NULL);
    }
...
void stratifysites::run ()  {
    ...
    cout <<  "## Informative site\n";
    cout <<  "## Type of informative site\n";
